...
let typecheck prog =
  Timer.wrap_duration "typechecking" (fun () -> Typecheck.tycheck_prog prog)
...
let coverage_check prog =
  Timer.wrap_duration "coverage checking" (fun () ->
      Coverage_check.coverage_check_prog prog)
...
let compile_for_importance_proposal ~ch model proposal =
  Timer.wrap_duration "emission" (fun () ->
      Compile.emit_prog_for_importance_proposal
...
let cmd_type_equality_check =
  Command.basic ~summary:"type equality check"
    (let open Command.Let_syntax in
     let%map_open filename = anon ("filename" %: Filename.arg_type)
     and first_type_name = anon ("first type name" %: string)
     and second_type_name = anon ("second type name" %: string) in
     fun () ->
       let result =
         let open Or_error.Let_syntax in
         let%bind prog = parse_file filename in
    ...
    (let open Command.Let_syntax in
     ...
     fun () ->
       let result =
         let open Or_error.Let_syntax in
         let%bind prog = parse_file filename in
         let%bind () = coverage_check prog in
         Ok ()
       in
       report_result result)
...
let cmd_compile_model =
  Command.basic ~summary:"compile (model)"
    (let open Command.Let_syntax in
     let%map_open filename = anon ("filename" %: Filename.arg_type)
     and output =
       flag "-output" (optional Filename.arg_type) ~doc:" output file"
     in
     fun () ->
       let result =
         let open Or_error.Let_syntax in
...
let cmd_compile_importance_proposal =
  Command.basic ~summary:"compile (guide)"
    (let open Command.Let_syntax in
     let%map_open model_name =
       flag "-model" (required Filename.arg_type) ~doc:" model"
     and proposal_name =
       flag "-guide" (required Filename.arg_type) ~doc:" guide"
     and output =
       flag "-output" (optional Filename.arg_type) ~doc:" output file"
     in
     fun () ->
       let result =
         ...
         let () =
           ...
           | Some output ->
               Out_channel.with_file output ~f:(fun ch ->
                   compile_for_importance_proposal ~ch model_iprog
                     proposal_iprog)
           | None ->
               compile_for_importance_proposal ~ch:stderr model_iprog
                 proposal_iprog
         in
